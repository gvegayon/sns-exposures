...
for (i in seq_along(networks)) {
  ...
  networks[[i]] <- lapply(schools, \(s) {
    induced_subgraph(n, which(V(networks[[i]])$school == s))
  })
...
for (s in seq_along(networks_by_school)) {
  ...
  exposures[[s]] <- merge(
    ...
    exposures[[s]][,.(id, year = as.integer(per), exposure_smoke)],
...
# Lagging exposures
dat_long[, exposure_smoke := shift(exposure_smoke, n = 1, type = "lag", fill = -1), by = id]
